...
<div class="row bg-title">
  <div class="col-lg-4 col-md-3 col-sm-3 col-xs-12">
    <h4 class="page-title dashboard">Geo Fence Logs</h4> 
  </div>
  ...
  @if(session()->get('success'))
    ...
    <a href="{{ url('geo-fence/'.$id) }}" class="btn btn-primary float-right" style="margin-bottom: 20px; margin-left:10px;">Back</a>
          ...
          <table id="example" class="uk-table uk-table-striped sms-table" style="width:100%">
            ...
            <tbody>
                  ...
                  <tr>
                      ...
                      <td>{{ $log->out_time }}</td>
                       <td>
                         @if($log->duration)
                          @if($log->duration > 60)                      
                                @php
                                        $makeTime = explode(':',date('H:i', mktime(0, $log->duration)))

                                @endphp
                          {{ $makeTime[0] }} hours {{ $makeTime[1] }} min

                          @else

                                {{ date('i', mktime(0, $log->duration)) }} min
                      ...
                      <td>
                          <form method="post" action="{{ url('remove-geo-fence-logs/'.$log->id) }}">
