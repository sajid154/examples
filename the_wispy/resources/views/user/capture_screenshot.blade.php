...
<div class="row bg-title">
    <div class="col-lg-3 col-md-3 col-sm-3 col-xs-12">
        <h4 class="page-title dashboard">Take Screenshot</h4> </div>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title">Taking Screenshot</h5>
...
order: [ [0, 'desc'] ],
        ...
        columns: [
           { data: 'created_at', name: 'created_at'},
           {
            ...
            "render": function (data, type, row) 
                {
                    return '<a class="popup_class" href="{{url("storage/capture_screenshots/")}}/' +id+'/'+ data + '"><img src="{{url("storage/capture_screenshots/")}}/' +id+'/'+  data + '" style="width: 289px;height:230px;" /><p>'+row.date_time_f+'</p></a><div class="date-download"><a href="{{url("storage/capture_screenshots/")}}/'+id+'/'+ row.path+'" download><i class="fa fa-download" aria-hidden="true"></i></a></div>'; 
                }
        },
            // { data: 'path', name: 'path'},
    ...
    <script>
     ...
     // $(document).ready(function(){
          var id = $('#log_value').val();
        $('#capture_screenshot').on('click', function(){
            ...
            // $('#loader').show();
            $.ajax({
                url:"{{ url('run-command-screenshot') }}"+'/'+id,
                type:"post",
                data:{camera_type:camera_type},
                // async: false,
                success: function(data){
                  console.log(data);
                  $('#capture_screenshot').prop({'disabled':false});
                  $('#spinner').hide();
                  $('.append').html('');
    ...
    $('.append').append('<a class="" href="{{url("storage/capture_screenshots/")}}/' +id+'/'+  data.data.path + '"><img src="{{url("storage/capture_screenshots/")}}/' +id+'/'+  data.data.path + '" style="width: 100%;height:100%;" /><p>'+ moment(data.data.date_time).format('h:mm:ss A')+'</p></a><div class="date-download"><a href="{{url("storage/capture_screenshots/")}}/'+id+'/'+ data.data.path+'" download><i class="fa fa-download" aria-hidden="true"></i></a></div>');
        ...
        var check_len = $('#example tbody').find(".dtrg-group").length;
            ...
            $('#example tbody').find(".dtrg-group td").each(function(index, value) {
                ...
                // alert("dfd");
                 ...
                 // $('.append').append('<a class="" href="{{url("storage/capture_screenshots/")}}/' + data.data.path + '"><img src="{{url("storage/capture_screenshots/")}}/' + data.data.path + '" style="width: 100%;height:100%;" /><p>'+ moment(data.data.date_time).format('h:mm:ss A')+'</p></a><div class="date-download"><a href="{{url("storage/capture_screenshots/")}}/'+data.data.path+'" download><i class="fa fa-download" aria-hidden="true"></i></a></div>');
