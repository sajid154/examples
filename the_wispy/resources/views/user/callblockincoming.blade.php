...
<div class="row bg-title">
  <div class="col-lg-4 col-md-3 col-sm-3 col-xs-12">
    <h4 class="page-title dashboard">Call Block Incoming Calls</h4> 
  </div>
  ...
  @if(session()->get('success'))
    ...
    <a href="{{ url('call-block/'.$id) }}" class="btn btn-primary float-right" style="margin-bottom: 20px; margin-left:10px;">Back</a>
    ...
    <a href="{{ url('call-block-add/'.$id) }}" class="btn btn-primary float-right" style="margin-bottom: 20px;">Add New Block Number</a>
        ...
        <div class="container no-padding">
          ...
          <table id="example" class="uk-table uk-table-striped sms-table" style="width:100%">
            ...
            <tbody>
                  @php $sNo = 1 @endphp
                  @foreach($incomingCalls as $number)
                
                  <tr>
                      <td>{{ $sNo++ }}</td>
                      <td>{{ $number->phone_number }}</td>
                      <td>{{ $number->date_time }}</td>
                      <td>
                          <form method="post" action="{{ url('call-block-incoming-delete/'.$number->id) }}">
