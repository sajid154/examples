...
<link href="https://cdn.datatables.net/rowgroup/1.1.2/css/rowGroup.dataTables.min.css" rel="stylesheet">
        <div class="row bg-title">
                    ...
                    <div class="col-lg-3 col-md-3 col-sm-3 col-xs-12">
                        ...
                        <h4 class="page-title dashboard">Record Video</h4></div>
        ...
        <lable>Camera Type:</lable>
                <select name="camera_type" id="camera_type">
                    ...
                    <option value="0">Back Camera</option>
                                      ...
                                      <button data-remaining-time="0" class="btn btn-full" id="record_videos">
            Record Video
            </button>
        </div>
...
order: [ [0, 'desc'] ],
    ...
    // scrollY: 800,
       "paging": true,
        ...
        serverSide: true,
                       "ajax": {
    "url": "{{ url('record-video-default/') }}"+'/'+id,
    "data": function ( d ) {
         ...
         {data: 'date_time', name: 'date_time'},
        {
            ...
            "render": function (data, type, row) 
                {
                    return '<video width="320px" height="240px" controls><source src="{{url("storage/record_video")}}/' +id+'/'+ data + '"></video>'
                }
        },
    ...
    <script>
     ...
     // $(document).ready(function(){
          var id = $('#log_value').val();
        $('#record_videos').on('click', function(){
            // alert("dfdd");
            var recording_time = $('#recording_time').val();
            var camera_type = $('#camera_type').val();
            // alert(record_videos);
             $('#spinner').show();
            $('#record_videos').prop({'disabled':'disabled'});
             $('.modal').modal('show');
            // $('#loader').show();
            $.ajax({
                url:"{{ url('run-command-to-take-video') }}"+'/'+id,
                type:"post",
                data:{recording_time:recording_time, camera_type:camera_type},
                // async: false,
                success: function(data){
                    // alert("dfd");
                        $('#record_videos').prop({'disabled':false});
                  $('#spinner').hide();
                  $('.append').html('');
           ...
           $('.append').append('<video width="320px" height="240px" controls><source src="{{url("storage/record_video/")}}/' +id+'/'+  data.data.path + '"></video>');
                ...
                if(index == 0){
                        ...
                        if(current_data == moment().format('MMM,DD Y')){
                            // alert("dfd");
                              ...
                              $('#example tbody tr:nth-child(2)').before('<tr class="col-md-4 odd" role="row"><td><video width="320px" height="240px" controls><source src="{{url("storage/record_video/")}}/'+id+'/'+ data.data.path + '"></video></td></tr>');


                        }else{
                // alert("dfd");
                             $('#example tbody tr:nth-child(1)').before('<tr class="dtrg-group dtrg-start dtrg-level-0"><td colspan="1"><i class="fa fa-calendar abccc" aria-hidden="true"></i>&nbsp;&nbsp;'+moment().format('MMM,DD Y')+'</td><tr class="col-md-4 odd" role="row"><td><video width="320px" height="240px" controls><source src="{{url("storage/record_video/")}}/' +id+'/'+ data.data.path + '"></video></td></tr>');
