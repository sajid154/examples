...
<div class="row bg-title">
  <div class="col-lg-12 col-md-4 col-sm-4 col-xs-12">
    <h4 class="page-title">Report by All Users  </h4> </div>
  </div>
  <!-- /.col-lg-12 -->
  <div class="row">
...
<div class="col-sm-12 col-md-4">
      <div class="setting-btn">
                <select class="form-control" id="user_type" >
                  ...
                  <option class="paid_user" name="user_type" value="all" selected>Open this select menu</option>

                  <option class="paid_user" name="user_type" value="paid_user">Paid User</option>

                  <option class="paid_user" name="user_type" value="active_paid_user">Active Paid User</option>

                  <option class="paid_user" name="user_type" value="inactive_paid_user">Inactive Paid User</option>

                  <option class="paid_user" name="user_type" value="free_user">Unpaid User</option>
...
<div class="col-sm-12 col-md-4">
      <div class="setting-btn">
            <select class="form-control" id="plan_type">
              <option class="paid_user" name="plan_type" value="all" selected>Select Plan</option>
              @foreach($plans as $plan)
              <option class="paid_user" name="plan_type" value="{{ $plan->id }}">{{ $plan->type.' '.$plan->title }}</option>
              @endforeach
            </select>
  </div>
</div>
...
<h5 class="col-md-2">End Date</h5>
 <div class="form-group col-md-3">
    ...
    <button type="text" id="btnFiterSubmitSearch" class="btn btn-info">Send Email</button>
    </div>
                    </div>
                    </div>
...
$.ajaxSetup({
  ...
  processing: "<img src='{{ asset('Loader-1-A.gif') }}' style='\
    ...
    processing: true,
        serverSide: true,
                ajax: {
            "url": "{{ url('report-by-all-users-default') }}",
            data: function (d) {
          d.start_date = $('#start_date').val();
          d.end_date = $('#end_date').val();
          d.user_type = $('#user_type').select().val();
          d.plan_type = $('#plan_type').select().val();
          }
        },
        columns: [
            { data: 'id', name: 'id'},
