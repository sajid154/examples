      @if(sizeof($result) > 0)
        ...
        @foreach($result as $row)
          <tr>
            ...
            <td>
          <a href="#" id="{{ $row->id }}" onclick="user_device_stats(this.id)" class="btn btn-xs btn-primary">
            <i class="glyphicon glyphicon-edit"></i>Stats
          </a>
        </td>
        <td>
          <a onclick="return confirm('Are you sure?')" href="{{ url('remove-device',$row->id) }}" target="_blank" class="btn btn-xs btn-primary"><i class="glyphicon glyphicon-edit"></i> Remove Records
          </a>
        </td>
          </tr>
        @endforeach
