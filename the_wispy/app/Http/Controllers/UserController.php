...
// return $exist;
    ...
    // dd($assigned_features);
       ...
       return $query->select(['id','title','type']);
		...
		$sms_test = SMS::select('smses.message','smses.number','smses.name as contact_name','smses.id','smses.date_time')
        ->where('smses.device_id', $id)
        ->orderBy('smses.date_time','desc')
        ->first();

        $recent_call =  CallLog::
        select('calllog.date_time','calllog.duration','calllog.type','name')
        ->where('calllog.device_id', $id)
        ->orderBy('calllog.date_time','desc')
        ->first();
        
		$recent_location = Location::where('device_id', $id)->latest()->first();

		$get_graph_data =  get_graph_data($request, $id);



		return view ('user.dashboard',compact('client_data','id','sms_test','recent_call','recent_location','get_graph_data'));
